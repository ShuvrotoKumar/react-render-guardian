...
function renderReasonToString(reason: RenderReason): string {
  const type = reason.type;
  let field = '';

  if (type === 'prop-changed' && reason.field) {
    field = reason.field;
  } else if (type === 'function-prop-changed' && reason.field) {
    field = reason.field;
  } else if (type === 'object-prop-changed' && reason.field) {
    field = reason.field;
  } else if (type === 'array-prop-changed' && reason.field) {
    field = reason.field;
  } else if (type === 'context-changed' && reason.contextName) {
    field = reason.contextName;
  }
...
function renderEventToString(event: RenderEvent, config: RenderGuardianConfig): string {
...
function generateSuggestions(event: RenderEvent): string {
...
function formatComponentStats(component: TrackedComponent, config: RenderGuardianConfig): string {
...
export function consoleReporter(config: RenderGuardianConfig = {}): void {
  ...
  const trackedComponents: TrackedComponent[] = store.getTrackedComponents();
  ...
  const hotComponents = trackedComponents
    .filter((component: TrackedComponent) => component.renderCount > maxRenders)
    .sort((a: TrackedComponent, b: TrackedComponent) => b.renderCount - a.renderCount);
...
function checkRuleViolation(
  ...
  rules: RenderRules
): { componentName: string; message: string; severity: 'error' | 'warn' } | null {
