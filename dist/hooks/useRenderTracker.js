...
import { renderStore } from '../types/types';
import { trackRender, detectWhyDidRender } from '../core/tracker';
export function useRenderTracker(Component, options = {}) {
    ...
    const whyDidRenderHistoryRef = useRef([]);
    useEffect(() => {
    ...
    const stats = {
        ...
        averageDuration: durationRef.current.length > 0
            ? durationRef.current.reduce((a, b) => a + b, 0) / durationRef.current.length
            : 0,
        maxDuration: durationRef.current.length > 0
            ? Math.max(...durationRef.current)
            : 0,
        minDuration: durationRef.current.length > 0
    ...
    const getRecentReasons = useCallback((limit) => {
...
//# sourceMappingURL=useRenderTracker.js.map